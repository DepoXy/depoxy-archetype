...
bashdx_configure_gvim_servername () {
  #  # The Vim server name to connect to. Aka open files in the same instance.
  #  # - E.g., `gvim --servername SAMPI --remote-silent <file file...>`
...
bashdx_wire_aliases_open_tracker () {
  ...
  if ! type "${bugs_alias}" > /dev/null 2>&1; then
    eval "alias ${bugs_alias}='sensible-open \
      \"https://wlweb.windlogics.com/radar/secure/Dashboard.jspa\"'"
  else
...
main () {
  if ${HOME_FRIES_PRELOAD}; then
    ...
    bashdx_configure_gvim_servername
    unset -f bashdx_configure_gvim_servername
  else
